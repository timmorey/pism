...
PetscErrorCode POMeltingParam3eqn::init(PISMVars &vars) {
  PetscErrorCode ierr;
//   ierr = verbPrintf(2, grid.com, "POMeltingParam3eqn::shelf_base_temperature called.\n");
//          CHKERRQ(ierr);
  ...
  ierr = temp.set_attrs("climate_forcing",
                        "absolute ocean temperature at ice shelf",
                        "Kelvin", ""); CHKERRQ(ierr);
  ierr = mass_flux.set_attrs("climate_forcing",
                       "ocean salinity at ice shelf",
                       "psu", "salinity"); CHKERRQ(ierr);
  ierr = temp_boundlayer.set_attrs("diagnostic",
                       "temperature at ice shelf base boundary layer",
                       "Kelvin", "temp_boundlayer"); CHKERRQ(ierr);
  ierr = salinity_boundlayer.set_attrs("diagnostic",
                       "salinity at ice shelf base boundary layer",
                       "psu", "salinity_boundlayer"); CHKERRQ(ierr);                       
...
PetscErrorCode POMeltingParam3eqn::calculate_boundlayer_temp_and_salt() {
...
//          CHKERRQ(ierr);
  ...
  ierr = mass_flux.begin_access(); CHKERRQ(ierr); // salinity here
  ...
  for (PetscInt i=grid.xs; i<grid.xs+grid.xm; ++i) {
    for (PetscInt j=grid.ys; j<grid.ys+grid.ym; ++j) {
      ...
      // FIXME: do not use mass flux as input field for salinity
      PetscReal sal_ocean = mass_flux(i,j);
      PetscReal press = rhow * -1.*shelfbaseelev/1000. + reference_pressure;
      potit(sal_ocean,thetao,press,reference_pressure, temp_insitu);
      PetscReal zice = -1*PetscAbs(shelfbaseelev);

      shelf_base_temp_salinity_3eq( sal_ocean, temp_insitu, zice, temp_base,
                                    sal_base);
...
//   PetscReal L = config.get("water_latent_heat_fusion"),
  ...
  for (PetscInt i=grid.xs; i<grid.xs+grid.xm; ++i) {
    for (PetscInt j=grid.ys; j<grid.ys+grid.ym; ++j) {
      ...
      // FIXME: do not use mass flux as input field for salinity
      sal_ocean = mass_flux(i,j);
      sal_base  = salinity_boundlayer(i,j);
      temp_base = temp_boundlayer(i,j) - 273.15; // to degC

      meltrate_3eq(rhow, rhoi, temp_base, sal_base, sal_ocean, meltrate_3eqn);
...
PetscErrorCode POMeltingParam3eqn::shelf_base_temp_salinity_3eq( PetscReal sal_ocean,
...
PetscErrorCode POMeltingParam3eqn::meltrate_3eq( PetscReal rhow, PetscReal rhoi,
