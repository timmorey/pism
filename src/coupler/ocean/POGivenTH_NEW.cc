...
PetscErrorCode POGivenTH::init(PISMVars &) {
  ...
  ierr = salinity.create(grid, salinity_name, false); CHKERRQ(ierr);
  ...
  ierr = salinity.set_attrs("climate_forcing",
  ...
  ierr = salinity.init(filename); CHKERRQ(ierr);
  ...
  if (temp.get_n_records() == 1 && salinity.get_n_records() == 1) {
...
PetscErrorCode POGivenTH::update(PetscReal my_t, PetscReal my_dt) {
  ...
  ierr = salinity.at_time(t); CHKERRQ(ierr);
...
PetscErrorCode POGivenTH::update(PetscReal t_years, PetscReal dt_years) {
  ...
  if (enable_time_averaging) {
    ierr = salinity.average(t, dt); CHKERRQ(ierr);
    ierr = temp.average(t, dt); CHKERRQ(ierr);
  } else {
    ierr = salinity.get_record_years(t); CHKERRQ(ierr);
...
//          CHKERRQ(ierr);
  ...
  ierr = salinity.begin_access(); CHKERRQ(ierr); // NOTE salinity instead of mass_flux
  ...
  for (PetscInt i=grid.xs; i<grid.xs+grid.xm; ++i) {
    for (PetscInt j=grid.ys; j<grid.ys+grid.ym; ++j) {
      ...
      PetscReal sal_ocean = salinity(i,j); //NOTE salinity instead of mass_flux
  ...
  ierr = salinity.end_access(); CHKERRQ(ierr);
...
//   PetscReal L = config.get("water_latent_heat_fusion"),
  ...
  for (PetscInt i=grid.xs; i<grid.xs+grid.xm; ++i) {
    for (PetscInt j=grid.ys; j<grid.ys+grid.ym; ++j) {
      ...
      sal_ocean = salinity(i,j); //NOTE salinity instead of mass_flux
...
//                         meltrate_3eqn,i,j); CHKERRQ(ierr);
  ...
  ierr = salinity.end_access(); CHKERRQ(ierr); //NOTE salinity instead of mass_flux
  ierr = salinity_boundlayer.end_access(); CHKERRQ(ierr);
  ierr = temp_boundlayer.end_access(); CHKERRQ(ierr);
